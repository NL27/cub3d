...
/*   Updated: 2022/04/01 14:02:44 by enijakow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	screen_render(t_screen *screen, t_map *map, t_vec2_and_angle pos)
{
	...
	while (x < screen_get_width(screen))
	{
		...
		screen_draw_slice(screen, x, screen_get_height(screen) / 2 - wall_height / 2, screen_get_height(screen) / 2 + wall_height / 2, ((hit.hit_block_x % 2) == (hit.hit_block_y % 2)) ? RGB_GRAY : RGB_WHITE);
		x = x + 1;
	}
}
